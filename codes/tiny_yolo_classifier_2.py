...
#===============================================================================
    ...
    def model(self, x):
        ...
        19 conv   nClasses  1 x 1 / 1   14 x 14 x 128      ->   14 x 14 x nClasses
        20 avg                          14 x 14 x nClasses ->   nClasses
        ...
        x = tf.layers.conv2d( x, kernel_size=(3,3), filters=16, padding='SAME', \
        ...
        x = tf.nn.leaky_relu( x, alpha=leak, name=layerName )
        if self.saveLayer:  self.layerOut[ layerName ] = x
        
        layerName = 'batchNorm' + layerIdx
        x = tf.layers.batch_normalization( x, training=self.isTraining, \
                                           name=layerName, trainable=True )

        if self.saveLayer:  self.layerOut[ layerName ] = x
        
        # Output size 224 x 224 x 16 (H x W x D).
        
#-------------------------------------------------------------------------------

        # Input size 224 x 224 x 16 (H x W x D).
        layerIdx = '1'
        ...
        # Output size 112 x 112 x 16 (H x W x D).
        
#-------------------------------------------------------------------------------
        
        # Input size 112 x 112 x 16 (H x W x D).
        layerIdx = '2'
        layerName = 'conv' + layerIdx
...
#-------------------------------------------------------------------------------
        ...
        # Output size 56 x 56 x 32 (H x W x D).
        
#-------------------------------------------------------------------------------
        
        # Input size 56 x 56 x 32 (H x W x D).
        layerIdx = '4'
        layerName = 'conv' + layerIdx
        x = tf.layers.conv2d( x, kernel_size=(1,1), filters=16, padding='SAME', \
                              name=layerName, bias_initializer=self.initB, \
                              kernel_initializer=self.initW, use_bias=True, trainable=True )
        if self.saveLayer:  self.layerOut[ layerName ] = x
        ...
        # Output size 56 x 56 x 16 (H x W x D).
        
#-------------------------------------------------------------------------------
        
        # Input size 56 x 56 x 16 (H x W x D).
        layerIdx = '5'
        layerName = 'conv' + layerIdx
...
#-------------------------------------------------------------------------------
        ...
        layerIdx = '9'
        layerName = 'conv' + layerIdx
        x = tf.layers.conv2d( x, kernel_size=(1,1), filters=32, padding='SAME', \
                              name=layerName, bias_initializer=self.initB, \
                              kernel_initializer=self.initW, use_bias=True, trainable=True )
        if self.saveLayer:  self.layerOut[ layerName ] = x
        ...
        # Output size 28 x 28 x 32 (H x W x D).
        
#-------------------------------------------------------------------------------
        
        # Input size 28 x 28 x 32 (H x W x D).
        layerIdx = '10'
        layerName = 'conv' + layerIdx
...
#-------------------------------------------------------------------------------
        ...
        layerIdx = '14'
        layerName = 'conv' + layerIdx
        x = tf.layers.conv2d( x, kernel_size=(1,1), filters=64, padding='SAME', \
                              name=layerName, bias_initializer=self.initB, \
                              kernel_initializer=self.initW, use_bias=True, trainable=True )
        if self.saveLayer:  self.layerOut[ layerName ] = x
        ...
        # Output size 14 x 14 x 64 (H x W x D).
        
#-------------------------------------------------------------------------------
        
        # Input size 14 x 14 x 64 (H x W x D).
        layerIdx = '15'
        layerName = 'conv' + layerIdx
...
#-------------------------------------------------------------------------------
        ...
        layerIdx = '18'
        layerName = 'conv' + layerIdx
        x = tf.layers.conv2d( x, kernel_size=(1,1), filters=128, padding='SAME', \
                              name=layerName, bias_initializer=self.initB, \
                              kernel_initializer=self.initW, use_bias=True, trainable=True )
        if self.saveLayer:  self.layerOut[ layerName ] = x
        ...
        # Output size 14 x 14 x 128 (H x W x D).
        
#-------------------------------------------------------------------------------
        
        # Input size 14 x 14 x 128 (H x W x D).
        layerIdx = '19'
        layerName = 'conv' + layerIdx
        x = tf.layers.conv2d( x, kernel_size=(1,1), filters=nClasses, padding='SAME', \
                              name=layerName, bias_initializer=self.initB, \
                              kernel_initializer=self.initW, use_bias=True, trainable=True )
        if self.saveLayer:  self.layerOut[ layerName ] = x
        ...
        # Output size 14 x 14 x nClasses (H x W x D).
        
#-------------------------------------------------------------------------------

        # Input size 14 x 14 x nClasses (H x W x D).
        layerIdx = '20'
        layerName = 'pooling' + layerIdx
        ...
        x = tf.layers.average_pooling2d( x, pool_size=(finalLayerH, finalLayerW), strides=1, \
...
#===============================================================================
    ...
    def loss(self, logits, labels):
        ...
        # classes.
        lossTensor = tf.losses.sigmoid_cross_entropy( logits=logits, \
...
#-------------------------------------------------------------------------------
        ...
        with tf.variable_scope( modelName, reuse=tf.AUTO_REUSE ):
            ...
            predProb = tf.sigmoid( predLogits )    # Predicted probabilities.
...
#===============================================================================
    ...
    def model(self, x):
        ...
        17 conv   512       3 x 3 / 1   14 x 14 x 64       ->   14 x 14 x 512
        18 conv   128       1 x 1 / 1   14 x 14 x 512      ->   14 x 14 x 128
        21 conv   128       3 x 3 / 1   14 x 14 x 128      ->   14 x 14 x 128
        22 conv   128       3 x 3 / 1   14 x 14 x 128      ->   14 x 14 x 128
        23 conv   128       3 x 3 / 1   14 x 14 x 128      ->   14 x 14 x 128
        24 conv   nAnchor * (nClasses + 5)  
                        1 x 1 / 1   14 x 14 x 128   ->  14 x 14 x nAnchor * (nClasses + 5)
           reshape      14 x 14 x nAnchor * (nClasses + 5)  ->  14 x 14 x nAnchor x (nClasses + 5)
        ...
        14 x 14 x nAnchor x (nClasses + 5) no. of filters. The cost function is 
...
#-------------------------------------------------------------------------------
        ...
        layerIdx = '24'
        layerName = 'conv' + layerIdx + self.detectorLayerID
        x = tf.layers.conv2d( x, kernel_size=(1,1), filters=nAnchors*(nClasses+5), \
        ...
        x = tf.reshape( x, shape=( -1, finalLayerH, finalLayerW, \
                                   nAnchors, nClasses + 5), name=layerName )
        
        if self.saveLayer: self.layerOut[ layerName ] = x
        
        # Output size 14 x 14 x nAnchors x (nClasses + 5).
...
#===============================================================================
    ...
    def loss(self, logits, labels):
        ...
        '''
        labels = tf.convert_to_tensor( labels )
        labels = tf.cast( labels, dtype=tf.float32 )    # Shape: (?, 14, 14, 5, 11)
        ...
        objectMask = tf.cast( objectMask, dtype=tf.bool )   # Shape: (?, 14, 14, 5)
        ...
        maskedLabels = tf.boolean_mask( labels, objectMask )    # Shape: (?, 11)
        #print( maskedLabels.get_shape() )       
        
        # Now only taking the corresponding regions of the logits.
        maskedLogits = tf.boolean_mask( logits, objectMask )    # Shape: (?, 11)

        # Now only taking the regions in the labels corresponding to the 0's in the mask.
        negMaskedLabels = tf.boolean_mask( labels, tf.logical_not( objectMask ) )

        # Now only taking the corresponding regions of the logits.
        negMaskedLogits = tf.boolean_mask( logits, tf.logical_not( objectMask ) )
...
#-------------------------------------------------------------------------------
        ...
        maskedLogitsXoffset = tf.sigmoid( maskedLogits[ :, -5 ] )   # Shape: (?,)
        maskedLogitsYoffset = tf.sigmoid( maskedLogits[ :, -4 ] )   # Shape: (?,)
        ...
        maskedLogitsConfidence = tf.sigmoid( maskedLogits[ :, -1 ] )   # Shape: (?,)
        ...
        negMaskedLogitsConfidence = tf.sigmoid( negMaskedLogits[ :, -1 ] )   # Shape: (?,)
        
        # Softmax for the one hot class prediction.
        maskedLogitsOneHotVec = tf.nn.softmax( maskedLogits[ :, : nClasses ] )   # Shape: (?, 6)
...
#-------------------------------------------------------------------------------
        ...
        lossX = tf.reduce_sum( tf.square( maskedLabelsXoffset - maskedLogitsXoffset ) )
        lossY = tf.reduce_sum( tf.square( maskedLabelsYoffset - maskedLogitsYoffset ) )

        # Corresponds to 2nd term of loss function in yolo '1' (not 2) paper (page 4).
        lossW = tf.reduce_sum( tf.square( maskedLabelsWoffset - maskedLogitsWoffset ) )
        lossH = tf.reduce_sum( tf.square( maskedLabelsHoffset - maskedLogitsHoffset ) )
        
        # Corresponds to 3rd term of loss function in yolo '1' (not 2) paper (page 4).
        lossConfidence = tf.reduce_sum( tf.square( maskedLabelsConfidence - \
                                                   maskedLogitsConfidence ) )

        # Corresponds to 4th term of loss function in yolo '1' (not 2) paper (page 4).
        lossNegConfidence = tf.reduce_sum( tf.square( negMaskedLabelsConfidence - \
                                                      negMaskedLogitsConfidence ) )
        
        # Corresponds to 5th term of loss function in yolo '1' (not 2) paper (page 4).
        lossOneHotVec = tf.reduce_sum( tf.square( maskedLabelsOneHotVec - \
...
#        # Loss per image.
#        meanLoss = totalLoss / ( tf.cast(tf.shape( logits )[0], tf.float32) )
...
#-------------------------------------------------------------------------------
        ...
        with tf.variable_scope( modelName, reuse=tf.AUTO_REUSE ):
            ...
            # List of model variables.
            listOfModelVars = [ v for v in tf.global_variables() ]
...
#-------------------------------------------------------------------------------
            ...
            predLogitsOneHotVec = tf.nn.softmax( predLogits[ :, :, :, :, : nClasses ] )
            predLogitsXoffset = tf.sigmoid( predLogits[ :, :, :, :, -5 ] )
            predLogitsYoffset = tf.sigmoid( predLogits[ :, :, :, :, -4 ] )
            predLogitsWoffset = tf.exp( predLogits[ :, :, :, :, -3 ] )
            predLogitsHoffset = tf.exp( predLogits[ :, :, :, :, -2 ] )    # Shape: (?, 14, 14, 5)
            ...
            anchorWtensor = tf.convert_to_tensor( anchorWarr, dtype=tf.float32 )
            anchorHlist = [ anc[1] for anc in anchorList ]
            anchorHarr = np.tile( anchorHlist, ( finalLayerH, finalLayerW, 1 ) )
            anchorHtensor = tf.convert_to_tensor( anchorHarr, dtype=tf.float32 )
            
            predLogitsWoffset = tf.multiply( predLogitsWoffset, anchorWtensor )
            predLogitsHoffset = tf.multiply( predLogitsHoffset, anchorHtensor )
            predLogitsConfidence = tf.sigmoid( predLogits[ :, :, :, :, -1 ] )

            # Now combining all these individual predlogits into one tensor.
            predLogitsXoffset = tf.expand_dims( predLogitsXoffset, axis=-1 )
            predLogitsYoffset = tf.expand_dims( predLogitsYoffset, axis=-1 )
            predLogitsWoffset = tf.expand_dims( predLogitsWoffset, axis=-1 )
            predLogitsHoffset = tf.expand_dims( predLogitsHoffset, axis=-1 )
            predLogitsConfidence = tf.expand_dims( predLogitsConfidence, axis=-1 )
            
            predResult = tf.concat( [ predLogitsOneHotVec, predLogitsXoffset, \
...
#-------------------------------------------------------------------------------
            ...
            saver = tf.train.Saver( listOfClassifierVars )
            saver.restore( sess, ckptPathClassifier )
            
            # Now initialize the rest of the layers and the optimizer variables.
            listOfOtherVars = list( set( tf.global_variables() ) - set( listOfClassifierVars ) )
                        
            sess.run( tf.variables_initializer( listOfOtherVars ) )
            ...
            saver = tf.train.Saver( listOfClassifierVars + listOfOtherVars )
            saver.max_to_keep = nSavedCkpt    # Save upto 5 checkpoints.
...
#-------------------------------------------------------------------------------
            ...
            predLogitsYoffset = tf.sigmoid( predLogits[ :, :, :, :, -4 ] )
            predLogitsWoffset = tf.exp( predLogits[ :, :, :, :, -3 ] )
            predLogitsHoffset = tf.exp( predLogits[ :, :, :, :, -2 ] )
...
#-------------------------------------------------------------------------------
            ...
            print( '\rScanning images. [{}/{}]'.format( i+1, nImgs ), end='' )
